...
bool OGLES2IntroducingPrint3D::InitApplication()
{
	...
	*/
	...
	while(*pC && ((unsigned int)(pC - pStart) < TextResource.Size()))
	{
		...
		unsigned int uiDataLen = pC - pLStart + 1;
...
bool OGLES2IntroducingPrint3D::RenderScene()
{
	...
	/*
		...
		demonstrates.
		Because we are not setting a projection or a modelview matrix the default projection 
		matrix is used.
	*/
	unsigned int uiTitleLang = ((ulCurrentTime / 1000) / (TITLE_TIME / 1000)) % eLang_Size;
	unsigned int uiNextLang  = (uiTitleLang + 1) % eLang_Size;
	unsigned int ulModTime   = ulCurrentTime % TITLE_TIME;
	...
	unsigned int uiTitleCol = (((unsigned int)(fTitlePerc * 255)) << 24) | 0x00FFFF;
	unsigned int uiNextCol  = (((unsigned int)(fNextPerc  * 255)) << 24) | 0x00FFFF;
	...
	*/
	m_Print3D.DisplayDefaultTitle(NULL, NULL, ePVRTPrint3DLogoIMG);
...
void OGLES2IntroducingPrint3D::RenderText()
{
	...
	/*
		Print3D can optionally be provided with user-defined projection and modelview matrices
		which allow custom layout of text. Here we are proving both a projection and modelview
		matrix. The projection matrix specified here uses perspective projection which will
		provide the 3D effect. The modelview matrix positions the the text in world space
	...
	*/
	...
	/*
		...
		text to the framebuffer. We are also fading out the text over a certain distance.
	*/
	float fPos, fFade;
	unsigned int uiCol;
	for(unsigned int uiIndex = 0; uiIndex < m_TextLines.GetSize(); ++uiIndex)
