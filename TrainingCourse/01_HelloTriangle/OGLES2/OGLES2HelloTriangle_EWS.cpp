/******************************************************************************
 ...
 @File         OGLES2HelloTriangle_EWS.cpp
...
int main(int argc, char **argv)
{
	...
	*/
	...
	/*
		Step 0 - Create our native window type that we can use for OpenGL ES output
	*/
	EWS_COORD windowPosition;
	EWS_SIZE windowSize;
	EWS_PIXELFORMAT ePixelFormat = EWS_PIXEL_FORMAT_ARGB_8888;

	windowPosition.iX = 0;
	windowPosition.iY = 0;
	windowSize.uiHeight= 480;
	windowSize.uiWidth = 640;

	EWS_DISPLAY EWSDisplay  = EWSOpenDisplay(EWS_DEFAULT_DISPLAY, 0);
	EWS_WINDOW  EWSWindow   = EWSCreateWindow(EWSDisplay, windowPosition, windowSize, ePixelFormat, EWS_ROTATE_0);
	...
	*/
	eglDisplay = eglGetDisplay((NativeDisplayType) EWSDisplay);
	...
	*/
	int i32Total_num_configs, j, num_config;
	EGLint value;
	EGLConfig* pConfigs;

	// Find out how many configs there are in total
	if(!eglChooseConfig(eglDisplay, pi32ConfigAttribs, NULL, 0, &i32Total_num_configs))
		goto cleanup;
	...
	if(!eglChooseConfig(eglDisplay, pi32ConfigAttribs, pConfigs, i32Total_num_configs, &num_config))
	{
		delete pConfigs;
		goto cleanup;
	}
	...
	{
		eglGetConfigAttrib(eglDisplay, pConfigs[j], EGL_NATIVE_VISUAL_ID, &value);

		if (value == ePixelFormat)
		{
			eglConfig = pConfigs[j];
			break;
		}
	}
	...
	/*
		...
		memory.
	*/
	eglSurface = eglCreateWindowSurface(eglDisplay, eglConfig, (EGLNativeWindowType) EWSWindow, NULL);
...
cleanup:
	...
	// Destroy our window
	EWSDestroyWindow(EWSWindow);
	EWSCloseDisplay(EWSDisplay);
	return 0;
}

/******************************************************************************
 End of file (OGLES2HelloTriangle_EWS.cpp)
******************************************************************************/
