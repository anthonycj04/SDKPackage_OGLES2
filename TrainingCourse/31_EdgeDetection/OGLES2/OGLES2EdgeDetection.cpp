...
bool OGLES2EdgeDetection::RenderScene()
{
	...
	// Render to our texture (bracketed for viewing convenience)
	{
		...
		for (unsigned int i=0; i<m_Scene.nNumMeshNode; i++)	DrawMesh(i);
		...
		if(m_bDiscard) // Was GL_EXT_discard_framebuffer supported?
		{
			...
			m_Extensions.glDiscardFramebufferEXT(GL_FRAMEBUFFER, 1, attachments);
		}
	...
	m_Print3D.DisplayDefaultTitle("Edge Detection", "", ePVRTPrint3DLogoIMG);
